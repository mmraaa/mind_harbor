...
function ApiConfigForm({ initial, onCancel, onSaved }: { initial: AdminApiConfig; onCancel: () => void; onSaved: () => Promise<void> }) {
  ...
  async function testConnection() { setTesting(true); setTestResult(initial.service_id === 'doodle_review' ? '正在发送文本测试，可能消耗少量 Token…' : '正在测试…'); try { const result = await adminApi.testApiConfig(initial.service_id); if (result.usage) { setUsage((current) => ({ ...current, prompt_tokens: current.prompt_tokens + result.usage!.prompt_tokens, completion_tokens: current.completion_tokens + result.usage!.completion_tokens, total_tokens: current.total_tokens + result.usage!.total_tokens, request_count: current.request_count + 1, failure_count: result.status === 'reachable' ? current.failure_count : current.failure_count + 1, remaining_tokens: current.remaining_tokens === null ? null : Math.max(0, current.remaining_tokens - result.usage!.total_tokens) })) } setTestResult(result.status === 'reachable' ? (initial.service_id === 'doodle_review' ? `文本测试通过，已收到模型回复，记录 ${result.usage?.total_tokens ?? 0} Token` : '连接可达，鉴权通过') : result.status === 'contract' ? '服务接口路径可达，但未发送内容，尚未执行模型推理验证。' : result.status === 'rate_limited' ? '服务接口可达，但当前触发了限流，请稍后再试。' : result.status === 'upstream_error' ? '服务接口可达，但上游暂时异常。' : result.status === 'invalid' ? (initial.service_id === 'doodle_review' ? '文本测试未收到有效回复，请检查鉴权、模型或请求格式。' : '地址可达，但鉴权或接口路径无效。') : '无法连接，请检查地址与网络') } catch (caught) { setTestResult(getErrorMessage(caught)) } finally { setTesting(false) } }
