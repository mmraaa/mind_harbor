  ...
  return <div className="admin-console">
    ...
    {loading ? <LoadingState /> : error ? <ErrorState message={error} onRetry={() => void load()} /> : overview && <>
      ...
      <section className="admin-api-panel">
        ...
        <div className="admin-api-list">{apiConfigs.map((service) => <article className="admin-api-row" key={service.service_id}><div><strong>{service.label}</strong><small>{service.model || '未设置模型'} · {service.api_key_configured ? service.api_key_masked : '未配置密钥'}</small></div><div className="admin-api-meta"><span className={`admin-status ${service.enabled ? 'admin-status--on' : 'admin-status--off'}`}>{service.enabled ? '已启用' : '已停用'}</span><small>请求 {service.usage.request_count} 次 · {service.usage.total_tokens.toLocaleString()} Token · 失败 {service.usage.failure_count} 次</small><button type="button" className="ghost-button admin-action" onClick={() => setEditingApi(service)}><Edit3 size={15} aria-hidden />配置</button></div></article>)}</div>
      </section>
...
function StudentForm({ initial, onSubmit, onCancel, saving }: { initial: Student; onSubmit: (payload: { name: string; risk_tags: string[]; is_enabled: boolean }) => Promise<void>; onCancel: () => void; saving: boolean }) {
  ...
  return <form className="admin-form" onSubmit={(event) => { event.preventDefault(); void onSubmit({ name, risk_tags: tags.split(/[、,，]/).map((tag) => tag.trim()).filter(Boolean), is_enabled: enabled }) }}><div className="admin-student-summary"><span className="admin-avatar"><UserRound size={17} aria-hidden /></span><div><strong>{initial.display_username || initial.name}</strong><small>{initial.username}{initial.name && initial.name !== initial.display_username ? ` · ${initial.name}` : ''}</small></div></div><label><span>姓名</span><input className="text-input" value={name} onChange={(event) => setName(event.target.value)} required /></label><label><span>风险标签</span><input className="text-input" value={tags} onChange={(event) => setTags(event.target.value)} placeholder="用逗号分隔，例如：关注、睡眠" /></label><label className="admin-toggle"><input type="checkbox" checked={enabled} onChange={(event) => setEnabled(event.target.checked)} /><span>允许该学生登录</span></label><div className="admin-dialog__actions"><button type="button" className="ghost-button" onClick={onCancel}>取消</button><button type="submit" className="primary-button" disabled={saving}>{saving ? '保存中…' : '保存账号'}</button></div></form>
}
