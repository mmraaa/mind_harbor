...
export const adminApi = {
  ...
  async testApiConfig(serviceId: string): Promise<{ service_id: string; status: 'reachable' | 'contract' | 'invalid' | 'rate_limited' | 'upstream_error' | 'unreachable' }> {
    const { data } = await api.post<{ service_id: string; status: 'reachable' | 'contract' | 'invalid' | 'rate_limited' | 'upstream_error' | 'unreachable' }>(`/admin/api-configs/${serviceId}/test`)
    return data
  },
  async validateDoodleApi(): Promise<{ service_id: string; status: 'verified' | 'reachable' | 'invalid' | 'rate_limited' | 'upstream_error' | 'unreachable'; usage?: { prompt_tokens: number; completion_tokens: number; total_tokens: number } }> {
    const { data } = await api.post<{ service_id: string; status: 'verified' | 'reachable' | 'invalid' | 'rate_limited' | 'upstream_error' | 'unreachable'; usage?: { prompt_tokens: number; completion_tokens: number; total_tokens: number } }>('/admin/api-configs/doodle_review/validate')
    return data
  },
